...
describe('auth primitives', () => {
        ...
        it('creates and fetches a user with normalized lookup', async () => {
                const user = await createUserAsync({ username: 'DinoMaster', password: 'secretpass1' }, connection);
        ...
        it('rejects duplicate usernames', async () => {
                ...
                await expect(() => createUserAsync({ username: 'repeat', password: 'password12' }, connection)).rejects
                        .toBeTruthy();
        });

        it('creates and validates sessions', async () => {
                const user = await createUserAsync({ username: 'SessionUser', password: 'password12' }, connection);
